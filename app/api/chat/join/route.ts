...
export async function POST(req: NextRequest) {
  ...
  const { data: room, error: roomError } = await supabase
    .from('chat_rooms')
    .select('id, name, type, anniversary_date, icon_url, mood_options, reaction_emojis, font_options, wallpaper_preset, wallpaper_url, bubble_mine_color, bubble_other_color')
    .eq('pin', pin.trim())
    .maybeSingle()

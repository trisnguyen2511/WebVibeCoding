...
// );
// alter table task_queue_snapshots enable row level security;
// create policy "allow anon rw" on task_queue_snapshots for all to anon using (true) with check (true);
...
export async function POST(req: NextRequest) {
  ...
  const res = await fetch(`${SUPABASE_URL}/rest/v1/task_queue_snapshots`, {
    method: 'POST',
    headers: {
      apikey: SUPABASE_KEY,
...
export async function GET(req: NextRequest) {
  ...
  const res = await fetch(
    `${SUPABASE_URL}/rest/v1/task_queue_snapshots?id=eq.${encodeURIComponent(id)}&select=data,synced_at`,
